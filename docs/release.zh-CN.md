...
```powershell
pnpm release:json -- --tag v0.2.11 --notes "更新说明（可选）"
```
...
```json
{
  ...
  "platforms": {
    "windows-x86_64": {
      "url": "https://github.com/rururunu/Anya/releases/download/v0.2.11/Anya_0.2.11_x64.msi",
      "signature": "<Anya_0.2.11_x64.msi.sig 文件的全部内容>"
    }
  }
}
```
...
2. Tag：`v0.2.11`（与 `--tag` 一致）
3. 填写 Release 说明（与 `notes` 可相同，给用户看）
4. 上传 **3 个文件**：
   - `Anya_0.2.11_x64.msi`
   - `Anya_0.2.11_x64.msi.sig`
