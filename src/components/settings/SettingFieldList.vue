<template>
  <section :class="hideHeader ? 'settings-embedded' : 'settings-page'">
    ...
    <template v-for="(group, groupIndex) in groups" :key="group.id">
          ...
          >
            ...
            >
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup v-for="themeGroup in builtInThemeGroups" :key="themeGroup.id">
                    <SelectLabel>{{ themeGroup.label }}</SelectLabel>
                    <SelectItem
                      v-for="option in themeGroup.options"
                      :key="option.value"
                      :value="option.value"
                    >
                      {{ option.label }}
              ...
              <div v-else-if="item.type === 'select-model'" class="space-y-1.5">
                ...
                <Select
                  ...
                  :model-value="modelSelectionForItem(item.id).id"
      ...
      <slot v-if="groupIndex === 0" name="after-first-group" />
    </template>
    <slot />
  </section>
</template>
...
import { buildFullThemeGroups } from "@/services/theme/catalog";
...
const emit = defineEmits<{
  ...
  "image-model-change": [value: unknown];
...
const builtInThemeGroups = computed(() => {
  ...
  return allGroups.map((group) => ({
    ...
    options: group.options.map((option) => ({
      value: group.id === "custom-themes" ? `custom:${option.value}` : `builtin:${option.value}`,
...
function modelSelectionForItem(itemId: string): ModelSelection {
  if (itemId === "multimodalModel") {
    return { id: settingStore.multimodalModel, provider: settingStore.multimodalModelProvider };
  }
  if (itemId === "imageModel") {
    return { id: settingStore.imageModel, provider: settingStore.imageModelProvider };
  }
  return { id: settingStore.chatModel, provider: settingStore.chatModelProvider };
}

function emitModelSelection(itemId: string, selection: ModelSelection) {
  if (itemId === "multimodalModel") emit("multimodal-model-change", selection);
  else if (itemId === "imageModel") emit("image-model-change", selection);
...
function handleModelSelection(itemId: string, value: unknown) {
  ...
  try {
    ...
    emitModelSelection(itemId, selection);
  } catch {
...
function selectedModelThinkingTierOptions(itemId: string) {
  const selectedId = modelSelectionForItem(itemId).id;
